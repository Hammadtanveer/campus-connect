            ...
            if (req.status == "pending" && isReceiver) {
                ...
                Column(horizontalAlignment = Alignment.Start) {
                    Button(onClick = {
                        ...
                        scope.launch {
                            // First update the request doc with notes (best-effort)
                            val db = FirebaseFirestore.getInstance()
                            val ref = db.collection("mentorship_requests").document(req.id)
                            ref.update("connectionNotes", notes.value)
                                .addOnSuccessListener {
                                    // Now accept - collect is a suspend operator so launch a coroutine inside listener
                                    scope.launch {
                                ...
                                .addOnFailureListener { e ->
                                    // If updating notes failed, still try to accept but inform user
                                    scope.launch {
                                        viewModel.acceptRequest(req.id).collect { r ->
                                            when (r) {
                                                is Resource.Success -> {
                                                    actionLoading.value = false
                                                    Toast.makeText(context, "Accepted (notes failed to save)", Toast.LENGTH_SHORT).show()
                                                    navController.popBackStack()
                                                }
                                                is Resource.Error -> {
                                                    actionLoading.value = false
