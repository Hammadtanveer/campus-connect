...
async function testUploads() {
    try {
        ...
        console.log("\n--- Testing AUTO upload (PDF) ---");
        try {
            const autoResult = await cloudinary.uploader.upload("test.pdf", {
                resource_type: "auto",
                public_id: "debug_pdf_auto_" + Date.now(),
                type: "upload"
            });
            console.log("AUTO (PDF) Upload Success:", autoResult.secure_url);
            console.log("Resolved Resource Type:", autoResult.resource_type);
        } catch (e) {
            console.error("AUTO (PDF) Upload Failed:", e.message);
        }
